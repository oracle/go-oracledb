...
//
// Deprecated: use oracle.OracleDriverProperties for application code.
...
//
// Deprecated: use oracle.OracleConnectionProperties for application code.
...
//
// Deprecated: use oracle.OracleLoggingConfig for application code.
...
//
// Deprecated: use oracle.NewOracleLoggingConfig for application code.
...
// database session.
//
// Deprecated: use oracle.OracleNLSParameters for application code.
...
//
// Deprecated: use oracle.OracleCredentials for application code.
...
//
// Deprecated: use oracle.OracleDriverConfig for application code.
...
//
// Deprecated: use oracle.NewOracleDriverConfig for application code.
	...
	for item := newConfig._allConfigurationField.Front(); item != nil; item = item.Next() {
		...
		if len(defaultValue) != 0 {
			if err := setFieldValueFromString(field, defaultValue); err != nil {
				Odl.Debug("Invalid default value", "field", field.name, "err", err)
			}
		}
	}
	return &newConfig
...
func setFieldValue(field *_fieldListElem, value any) error {
	if !field.elementValue.CanSet() {
		return NewOracleError(InvalidConnectionParameter, nil, value, field.name, "settable field")
	}

	targetType := field.elementValue.Type()
	valueToSet := reflect.ValueOf(value)
	if !valueToSet.IsValid() {
		return NewOracleError(InvalidConnectionParameter, nil, value, field.name, targetType.String())
	}
...
func QueryStringToMap(s string) (map[string]string, error) {
	...
	for _, part := range queryParts {
		...
		if len(keyValuePair) != 2 {
			return nil, NewOracleError(NamingParseFailed, fmt.Errorf("error parsing connection property [%s]", part))
		} else {
...
func validateZeroOrPositive(value reflect.Value, valueName string) (any, error) {
	...
	case reflect.String:
		...
		if err != nil || parsedValue < 0 {
			return nil, NewOracleError(InvalidConnectionParameter, nil,
				value.String(),
				valueName,
				[]string{"int64"})
		}
...
func validateLanguage(value reflect.Value, valueName string) (any, error) {
	...
	if value.Kind() == reflect.String {
		if len(value.String()) != 0 {
			...
			if err != nil {
				Odl.Debug(fmt.Sprintf("Error parsing connection property [%s], value [%s]", valueName, value))
			} else {
				return lang, nil
			}
		}
	...
	return language.Tag{}, NewOracleError(
		InvalidConnectionParameter,
		nil,
		value,
...
func validateLogonModeValue(value reflect.Value, valueName string) (any, error) {
	if value.Kind() == reflect.String {
		...
		mode, err := GetLogonModeFromString(value.String())
		if err == nil {
