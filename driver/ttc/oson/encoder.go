...
//     float32, float64, []byte, time.Time, common.JSONNumber, json.Number, map[string]any, []any.
//
// Output:
// - common.B1Array containing the encoded OSON document.
//
// Errors:
// - common.OsonEncodingError for an unsupported value or an OSON encoding/size limit failure.
func Encode(value any) (common.B1Array, error) {
	enc := newOsonEncoder()
...
type osonEncoder struct {
	...
	treeSegmentBytes common.B1Array
}
...
func (enc *osonEncoder) encode(value any) (common.B1Array, error) {
...
func (enc *osonEncoder) encodeScalarDocument(value any) (common.B1Array, error) {
...
func (enc *osonEncoder) emitScalarDocument() common.B1Array {
	var out osonWriteBuffer
	out.writeUB4(common.UB4(osonMagicPrefix | int(enc.version)))
	out.writeUB2(enc.flags)
	if enc.flags&osonFlagTreeSegmentSizeUB4Mask != 0 {
		out.writeUB4(common.UB4(len(enc.treeSegmentBytes)))
	} else {
		out.writeUB2(common.UB2(len(enc.treeSegmentBytes)))
	}
...
func (enc *osonEncoder) encodeContainer(value any) (common.B1Array, error) {
...
// flag.
func (enc *osonEncoder) emitContainerDocument() common.B1Array {
	var out osonWriteBuffer
	out.writeUB4(common.UB4(osonMagicPrefix | int(enc.version)))
	out.writeUB2(enc.flags)
...
func (enc *osonEncoder) addFieldName(name string) error {
	...
	if byteLen > osonMaxSecondaryDictKeyLength {
		...
		return common.NewOracleError(common.OsonEncodingError, cause, byteLen)
	}

	field := &fieldNameEntry{
		name: name,
		raw:  common.B1Array([]byte(name)),
	}
...
func (enc *osonEncoder) unsupportedValueError(operation, role string, value any) error {
	...
	return common.NewOracleError(common.OsonEncodingError, cause, fmt.Sprintf("%T", value))
}
...
func (enc *osonEncoder) bufferPatchError(operation string, err error) error {
	...
	return common.NewOracleError(common.OsonEncodingError, err, nil)
}
...
func (enc *osonEncoder) sortedObjectMembers(value map[string]any) ([]objectMember, error) {
	...
	for key, child := range value {
		...
		if !ok {
			...
			return nil, common.NewOracleError(common.OsonEncodingError, cause, key)
		}
...
func (enc *osonEncoder) writeScalarNode(tree *osonWriteBuffer, value any) error {
	...
	case []byte:
		return enc.writeBinaryScalar(tree, common.B1Array(v))
	case time.Time:
...
func (enc *osonEncoder) writeStringScalar(tree *osonWriteBuffer, value string) error {
	...
	if len(raw) > _maxUB4 {
		...
		return common.NewOracleError(common.OsonEncodingError, cause, len(raw))
	}

	switch {
	case len(raw) <= int(osonOpShortStringMax):
		tree.writeUB1(common.UB1(len(raw)))
	case len(raw) < _maxUB1:
		tree.writeUB1(osonOpStringUB1)
		tree.writeUB1(common.UB1(len(raw)))
	case len(raw) < _maxUB2:
		tree.writeUB1(osonOpStringUB2)
		tree.writeUB2(common.UB2(len(raw)))
	default:
		tree.writeUB1(osonOpStringUB4)
		tree.writeUB4(common.UB4(len(raw)))
	}
	tree.writeBytes(common.B1Array(raw))
	return nil
}

// writeSignedIntScalar writes a signed integer scalar.
func (enc *osonEncoder) writeSignedIntScalar(tree *osonWriteBuffer, value int64, opcodeMask common.UB1, platformSize common.UB1) error {
	...
	opcode := opcodeMask | common.UB1(len(payload))
...
func (enc *osonEncoder) writeUnsignedIntScalar(tree *osonWriteBuffer, value uint64) error {
	...
	if len(payload) == 0 {
		...
		return common.NewOracleError(common.OsonEncodingError, cause, nil)
	}
	if len(payload) <= _compactOracleNumberMaxPayloadLen {
		opcode := osonOpCompactOracleNumberPrefix | common.UB1(len(payload)-1)
	...
	if len(payload) > _maxUB1 {
		...
		return common.NewOracleError(common.OsonEncodingError, cause, len(payload))
	}
	...
	tree.writeUB1(common.UB1(len(payload)))
	tree.writeBytes(payload)
	return nil
}
...
func (enc *osonEncoder) writeStringNumberScalar(tree *osonWriteBuffer, value string) error {
	if value == "" || strings.TrimSpace(value) != value || !stdjson.Valid([]byte(value)) {
		...
		return common.NewOracleError(common.OsonEncodingError, cause, value)
	}
...
func (enc *osonEncoder) writeBinaryScalar(tree *osonWriteBuffer, value common.B1Array) error {
	if len(value) > _maxUB4 {
		...
		return common.NewOracleError(common.OsonEncodingError, cause, len(value))
	}

	if len(value) < _maxUB2 {
		tree.writeUB1(osonOpBinaryUB2)
		tree.writeUB2(common.UB2(len(value)))
	} else {
		tree.writeUB1(osonOpBinaryUB4)
		tree.writeUB4(common.UB4(len(value)))
	}
...
func containerOpcode(base common.UB1, count, childOffsetSize int) common.UB1 {
	opcode := base
...
func (enc *osonEncoder) writePrimaryDictionaryHeader(out *osonWriteBuffer, heapSize int) {
	...
	if heapSize > _maxUB2 {
		out.writeUB4(common.UB4(heapSize))
	} else {
		out.writeUB2(common.UB2(heapSize))
	}
}
...
func (enc *osonEncoder) writeUint(out *osonWriteBuffer, size, value int) {
	switch size {
	case osonUB1Size:
		out.writeUB1(common.UB1(value))
	case osonUB2Size:
		out.writeUB2(common.UB2(value))
	default:
		out.writeUB4(common.UB4(value))
	}
}
...
func (b *osonWriteBuffer) writeUB1(value common.UB1) {
	b.data = append(b.data, byte(value))
}

// writeUB2 appends one big-endian unsigned 2-byte integer.
func (b *osonWriteBuffer) writeUB2(value common.UB2) {
...
func (b *osonWriteBuffer) writeUB4(value common.UB4) {
...
func (b *osonWriteBuffer) writeBytes(value common.B1Array) {
...
func (b *osonWriteBuffer) reserve(length int) int {
	start := len(b.data)
	b.data = append(b.data, make(common.B1Array, length)...)
	return start
}
...
func (b *osonWriteBuffer) writeContainerCount(count int) {
	switch {
	case count >= _maxUB2:
		b.writeUB4(common.UB4(count))
	case count >= _maxUB1:
		b.writeUB2(common.UB2(count))
	default:
		b.writeUB1(common.UB1(count))
	}
}
...
func (b *osonWriteBuffer) patchUint(offset, width, value int) error {
	...
	switch width {
	case osonUB1Size:
		b.data[offset] = byte(common.UB1(value))
	case osonUB2Size:
...
func (b *osonWriteBuffer) bytes() common.B1Array {
	out := make(common.B1Array, len(b.data))
	copy(out, b.data)
	return out
}
