...
func (rxd *tTIrxd) setCurrentOACs(oacs []common.Marshallable) {
	rxd.oacs = oacs
}
...
func (rxd *tTIrxd) MarshalTo(ctx context.Context, engine common.Marshaller) error {
	...
	for i := 0; i < bindCount; i++ {
		...
		if requestedType == common.DtyRSet {
			if err := engine.MarshalUB1(ctx, common.UB1(1)); err != nil {
...
func (rxd *tTIrxd) marshalNamedTypeBind(ctx context.Context, engine common.Marshaller, image common.B1Array) error {
	...
	if err := engine.MarshalUB4(ctx, common.UB4(len(image))); err != nil {
		return common.NewOracleError(common.FailMarshal, err, TTCMsgTypeDescription[rxd.GetMsgCode()])
	}
...
*/
func (rxd *tTIrxd) _unmarshalColumn(ctx context.Context, dtyType common.DtyType, mar common.Marshaller, col int) error {
	switch dtyType {
	case common.DtyCur:
...
func (rxd *tTIrxd) _unmarshalNamedTypeColumn(ctx context.Context, mar common.Marshaller, col int) error {
...
Errors:
  - Returns an error when the prefetched column payload cannot be read from the wire.
*/
func (rxd *tTIrxd) _unmarshalBlobColumn(ctx context.Context, mar common.Marshaller, col int, dtyType common.DtyType) error {
	// length
	lob := &LobColumnContext{}
	var err error
	if lob.LobLength, err = mar.UnmarshalUB4(ctx); err != nil {
