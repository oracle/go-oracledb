...
func TestTTIuds17_UnMarshalFrom_Success(t *testing.T) {
	...
	typed := obj.(*tTIuds17)
	if len(typed.domainSchema) != 0 {
		t.Errorf("expected domainSchema '', got %q", typed.domainSchema)
	}
	if len(typed.domainName) != 0 {
		t.Errorf("expected domainName '', got %q", typed.domainName)
	}
}
...
func TestTTIuds17_UnMarshalFrom_Fail(t *testing.T) {
	...
	faults := []faultyTest{
		{"Fail on TTIuds UmarshalFrom", 1},
		{"Fail on domainSchema", 22},
		{"Fail on domainName", 23},
	}
	payload := makeTtiudsUnmarshalPayload(validTtiudsUnmarshalDump)
