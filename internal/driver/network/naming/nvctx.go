...
func ExtractConnectionContext(root *Node) (*ConnectionContext, error) {
	if root == nil {
		return nil, common.NewOracleError(oracleErrors.NamingContextError, nil, "", "root")
	}
	...
	default:
		return nil, common.NewOracleError(oracleErrors.NamingContextError, nil, root.Name, "root")
	}
	...
	for _, child := range node.Children {
		...
		case "LOAD_BALANCE":
			if descList.LoadBalance, parsingError = normalizeBoolean(child.Value); parsingError != nil {
				return nil, common.NewOracleError(oracleErrors.NamingContextError, parsingError, child.Value, child.Name)
			}
		case "FAILOVER":
		...
		default:
			return nil, common.NewOracleError(oracleErrors.NamingContextError, nil, child.Name, "DESCRIPTION_LIST")
		}
	}
...
func extractDescription(node *Node) (*Description, error) {
	...
	for _, child := range node.Children {
		...
		default:
			return nil, common.NewOracleError(oracleErrors.NamingContextError, nil, child.Name, "DESCRIPTION")
		}
	}
...
func extractAddressList(node *Node) (*AddressList, error) {
	...
	for _, child := range node.Children {
		...
		default:
			return nil, common.NewOracleError(oracleErrors.NamingContextError, nil, child.Name, "ADDRESS_LIST")
		}
	}
...
func extractAddress(node *Node) (Address, error) {
	...
	for _, child := range node.Children {
		...
		switch name {
		case "PROTOCOL":
			if addr.Protocol, parsingError = driverCommon.NormalizeProtocol(child.Value); parsingError != nil {
...
func extractSecurity(node *Node) (*Security, error) {
	...
	for _, child := range node.Children {
		...
		default:
			return nil, common.NewOracleError(oracleErrors.NamingContextError, nil, child.Name, "SECURITY")
		}
	}
