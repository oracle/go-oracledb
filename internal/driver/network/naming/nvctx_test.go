...
func TestHelpersAndMethods(t *testing.T) {
	...
	t.Run("normalizeBoolean", func(t *testing.T) {
		...
		if sqle.ErrorCode() != string(oracleErrors.InvalidConnectionParameter) {
			t.Fatalf("expected error code %s, got %s", oracleErrors.InvalidConnectionParameter, sqle.ErrorCode())
		}
		if !strings.Contains(sqle.Error(), "InvalidConnectionParameter") && !strings.Contains(sqle.Error(), string(oracleErrors.InvalidConnectionParameter)) {
