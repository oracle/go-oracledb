...
func parsePKCS8EncryptedPrivateKey(block *pem.Block, password []byte) (interface{}, error) {
	...
	if block.Type != "ENCRYPTED PRIVATE KEY" {
		return nil, common.NewOracleError(oracleErrors.InvalidNetworkExpectedValue, nil, "PEM block type", block.Type, "ENCRYPTED PRIVATE KEY")
	}
	...
	if _, err := asn1.Unmarshal(block.Bytes, &encInfo); err != nil {
		return nil, common.NewOracleError(oracleErrors.PKCS8ParseFailed, err)
	}

	if !encInfo.Algorithm.OID.Equal(oidPBES2) {
		return nil, common.NewOracleError(oracleErrors.InvalidNetworkValue, nil, "PKCS#8 encryption algorithm OID", encInfo.Algorithm.OID)
	}
	...
	if _, err := asn1.Unmarshal(encInfo.Algorithm.Params.FullBytes, &pbes2); err != nil {
		return nil, common.NewOracleError(oracleErrors.PKCS8ParametersParseFailed, err, "PBES2")
	}
	if !pbes2.KDF.OID.Equal(oidPBKDF2) {
		return nil, common.NewOracleError(oracleErrors.InvalidNetworkValue, nil, "PBES2 KDF OID", pbes2.KDF.OID)
	}
	...
	if _, err := asn1.Unmarshal(pbes2.KDF.Params.FullBytes, &kdf); err != nil {
		return nil, common.NewOracleError(oracleErrors.PKCS8ParametersParseFailed, err, "PBKDF2")
	}
	...
	if _, err := asn1.Unmarshal(pbes2.Encryption.Params.FullBytes, &iv); err != nil {
		return nil, common.NewOracleError(oracleErrors.PKCS8InitializationVectorParseFailed, err)
	}
	...
	if err != nil {
		return nil, common.NewOracleError(oracleErrors.PKCS8DecryptFailed, err)
	}
...
func hashForPBKDF2PRF(prf algorithmIdentifier) (func() hash.Hash, error) {
	...
	default:
		return nil, common.NewOracleError(oracleErrors.InvalidNetworkValue, nil, "PBKDF2 PRF OID", prf.OID)
	}
}
...
func keyLengthForOID(oid asn1.ObjectIdentifier) (int, error) {
	...
	default:
		return 0, common.NewOracleError(oracleErrors.InvalidNetworkValue, nil, "PKCS#8 encryption OID", oid)
	}
}
...
func decrypt(oid asn1.ObjectIdentifier, key, iv, ciphertext []byte) ([]byte, error) {
	...
	default:
		return nil, common.NewOracleError(oracleErrors.InvalidNetworkValue, nil, "PKCS#8 encryption OID", oid)
	}
}
...
func stripPKCS7Padding(data []byte, blockSize int) ([]byte, error) {
	...
	if padLen == 0 || padLen > blockSize || padLen > len(data) {
		return nil, common.NewOracleError(oracleErrors.PKCS7PaddingInvalid, nil)
	}
