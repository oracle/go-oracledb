...
func TestConnectionTransaction_ConcurrentTerminationExecutesOnce(t *testing.T) {
	...
	streamer := &transactionBlockingStreamer{
		mockStreamer: &mockStreamer{pullMsg: &mockOer{}},
		flushStarted: make(chan struct{}),
		allowFlush:   make(chan struct{}),
	}
	var allowFlushOnce sync.Once
	allowFlush := func() { allowFlushOnce.Do(func() { close(streamer.allowFlush) }) }
	defer allowFlush()
	...
	// Release the guard so the first terminal exchange can reach its
	// fake-streamer barrier. The other terminal call must remain outside the
	// stream until the first exchange is drained.
	hold()
	<-streamer.flushStarted
	allowFlush()
...
type transactionBlockingStreamer struct {
	*mockStreamer
	flushStarted chan struct{}
	allowFlush   chan struct{}
}

func (streamer *transactionBlockingStreamer) Flush(ctx context.Context) error {
	close(streamer.flushStarted)
	<-streamer.allowFlush
	return streamer.mockStreamer.Flush(ctx)
}
