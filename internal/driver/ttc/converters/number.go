...
func validateNumberWireLength(inputData []byte) error {
	if len(inputData) > _numberWireMaxLen {
		return common.NewOracleError(
			oracleErrors.ConverterExpectedFormat,
			nil,
			"NUMBER",
			"Decode",
			driverCommon.ReasonInvalidLength,
...
func validateNumberMetadata(precision, scale int) error {
	...
	if precision > _numberMaxPrecision {
		return common.NewOracleError(
			...
			"NUMBER",
			"Decode",
			driverCommon.ReasonPrecisionExceeded,
...
func DecodeInt(inputData []byte) (int64, error) {
	...
	if exponent < 0 {
		return 0, common.NewOracleError(oracleErrors.ConverterExpectedFormat, nil, "NUMBER", "Decode", driverCommon.ReasonInvalidFormat, "integer")
	}
	...
	if exponent > 0 {
		if exponent >= len(_pow10) {
			return 0, common.NewOracleError(oracleErrors.ConverterRange, nil, "NUMBER", "Decode", driverCommon.ReasonOutOfRange, math.MinInt64, int64(maxInt64))
		}
...
- None
*/
func EncodeInt(v driver.Value) (driverCommon.B1Array, error) {
...
func encodeIntegerDigitsStrict(mantissa []byte, negative bool) driverCommon.B1Array {
...
- None
*/
func EncodeUInt(v driver.Value) (driverCommon.B1Array, error) {
...
*/
func EncodeBinary(v driver.Value) (driverCommon.B1Array, error) {
...
- None
*/
func EncodeNull(_ driver.Value) (driverCommon.B1Array, error) {
	return nil, nil
}
...
- nil
*/
func EncodeBoolean(v driver.Value) (driverCommon.B1Array, error) {
	b := v.(bool)
	if b {
...
*/
func EncodeBooleanAsNumber(v driver.Value) (driverCommon.B1Array, error) {
	b := v.(bool)
	if b {
		return EncodeInt(1)
	}
...
func EncodeDecimal(v driver.Value) ([]byte, error) {
	...
	if math.IsNaN(num) || math.IsInf(num, _signAny) {
		return nil, common.NewOracleError(oracleErrors.ConverterExpectedFormat, nil, "NUMBER", "Encode", driverCommon.ReasonInvalidValue, "finite number")
	}
	abs := math.Abs(num)
	if abs >= 1e126 || (abs > 0 && abs < 1e-130) {
		return nil, common.NewOracleError(oracleErrors.ConverterRange, nil, "NUMBER", "Encode", driverCommon.ReasonOutOfRange, "[1e-130, 1e126)")
	}
...
func FromNumber(inputData []byte) (mantissa uint64, negative bool, exponent int, mantissaDigits int, err error) {
	if len(inputData) == 0 {
		return 0, false, 0, 0, common.NewOracleError(oracleErrors.ConverterEmptyInput, nil, "NUMBER", "Decode")
	}
	...
	if negative && inputData[len(inputData)-1] == _negTerminatorByte {
		if len(inputData) == 1 {
			...
			return 0, negative, exponent, mantissaDigits,
				common.NewOracleError(oracleErrors.ConverterExpectedFormat, nil,
					"NUMBER", "Decode", driverCommon.ReasonInvalidFormat, "_negTerminatorByte")
		}
	...
	for p, digit100 := range buf {
		...
		if d > _packedPairBase-1 {
			return 0, negative, exponent, mantissaDigits, common.NewOracleError(oracleErrors.ConverterExpectedFormat, nil, "NUMBER", "Decode", driverCommon.ReasonInvalidFormat, "packed-digit[0..99]")
		}
		...
		if highDigit < 0 || highDigit > _maxSingleDigit || lowDigit < 0 || lowDigit > _maxSingleDigit {
			return 0, negative, exponent, mantissaDigits, common.NewOracleError(oracleErrors.ConverterExpectedFormat, nil, "NUMBER", "Decode", driverCommon.ReasonInvalidFormat, "single-digit[0..9]")
		}
		pair := int(d)
		// Overflow check for mantissa = mantissa*100 + pair
		if mantissa > (math.MaxUint64-uint64(pair))/uint64(_packedPairBase) {
			return 0, negative, exponent, mantissaDigits, common.NewOracleError(oracleErrors.ConverterExpectedFormat, nil, "NUMBER", "Decode", driverCommon.ReasonPrecisionExceeded, "uint64 mantissa")
		}
...
func _fromNumberBig(inputData []byte) (mantissa *big.Int, negative bool, exponent int, mantissaDigits int, err error) {
	if len(inputData) == 0 {
		return nil, false, 0, 0, common.NewOracleError(oracleErrors.ConverterEmptyInput, nil, "NUMBER", "Decode")
	}
	if err = validateNumberWireLength(inputData); err != nil {
	...
	for p, digit100 := range buf {
		...
		if d < 0 || d > _packedPairBase-1 {
			return nil, negative, exponent, mantissaDigits, common.NewOracleError(oracleErrors.ConverterExpectedFormat, nil, "NUMBER", "Decode", driverCommon.ReasonInvalidFormat, "packed-digit[0..99]")
		}
		highDigit := int(d / _decimalBase)
		lowDigit := int(d % _decimalBase)
		if highDigit < 0 || highDigit > _maxSingleDigit || lowDigit < 0 || lowDigit > _maxSingleDigit {
			return nil, negative, exponent, mantissaDigits, common.NewOracleError(oracleErrors.ConverterExpectedFormat, nil, "NUMBER", "Decode", driverCommon.ReasonInvalidFormat, "single-digit[0..9]")
		}
...
func addDigitToMantissa(current uint64, digit int) (uint64, error) {
	if digit < 0 || digit > _maxSingleDigit {
		return current, common.NewOracleError(oracleErrors.ConverterExpectedFormat, nil, "NUMBER", "Decode", driverCommon.ReasonInvalidFormat, "single-digit[0..9]")
	}
	const maxMantissa = (1<<64 - 1) / _decimalBase // Prevent multiplication overflow
	if current > maxMantissa {
		return current, common.NewOracleError(oracleErrors.ConverterExpectedFormat, nil, "NUMBER", "Decode", driverCommon.ReasonPrecisionExceeded, "uint64 mantissa")
	}
