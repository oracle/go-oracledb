...
var decimalNumberTestCases = []float64TestCase{
	...
	{name: "empty", wire: []byte{}, precision: 1, scale: 0, expected: 0, wantErr: true, expCode: oracleErrors.ConverterEmptyInput},
...
func assertNumberConverterError(t *testing.T, err error, wantReason string) {
	...
	if !errors.As(err, &sqlErr) {
		t.Fatalf("error is not oracleErrors.SQLError: %T: %v", err, err)
	}

	if got := sqlErr.ErrorCode(); got != string(oracleErrors.ConverterExpectedFormat) {
		t.Fatalf("error code=%s, want=%s: %v", got, oracleErrors.ConverterExpectedFormat, err)
	}
...
func TestNumber_ErrorCodes(t *testing.T) {
	...
	cases := []errCase{
		...
		},
		{
			...
			want: oracleErrors.ConverterExpectedFormat,
		},
		{
		...
		},
		{
			...
			want: oracleErrors.ConverterRange,
		},
		{
			name: "DecodeInt overflow -",
			fn: func() error {
...
func TestNumber_EncodeDecimal_StrictErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{"nan", math.NaN(), oracleErrors.ConverterExpectedFormat},
		{"pos_inf", math.Inf(1), oracleErrors.ConverterExpectedFormat},
		{"neg_inf", math.Inf(-1), oracleErrors.ConverterExpectedFormat},
		{"too_large_pos", 1e126, oracleErrors.ConverterRange},
		{"too_large_neg", -1e126, oracleErrors.ConverterRange},
		{"too_small_pos", 1e-131, oracleErrors.ConverterRange},
		{"too_small_neg", -1e-131, oracleErrors.ConverterRange},
		{"subnormal_pos", math.SmallestNonzeroFloat64, oracleErrors.ConverterRange},
		{"subnormal_neg", -math.SmallestNonzeroFloat64, oracleErrors.ConverterRange},
	}
	for _, c := range cases {
...
// TestEncodeNull verifies that a nil value is encoded as a nil byte slice
...
// TestEncodeBooleanAsNumber verifies the Oracle NUMBER wire values used for
// false and true when booleans are encoded as numbers.
