...
type qualifiedSQLStatement struct {
	query    driverCommon.B1Array // the original SQL query
	kind     sqlKind              // the kind of the SQL query
	binds    *bindDetails         // the binds of the SQL query
	cursorId driverCommon.SB4     // id of the cursorId while this query is executed
}
...
func newQualifiedSQLStatement(query string) (*qualifiedSQLStatement, error) {
	...
	return &qualifiedSQLStatement{query: driverCommon.StringToB1Array(query), kind: k, binds: b}, nil
}
...
func _classifySQL(q string) (sqlKind, error) {
	...
	if s == "" {
		...
		return 0, common.NewOracleError(oracleErrors.StatementExecutionFailed, nil, "query", "empty SQL")
	}
	...
	if !ok {
		...
		return 0, common.NewOracleError(oracleErrors.StatementExecutionFailed, nil, "qualifiedSQLStatement", "unrecognised query")
	}
	return k, nil
