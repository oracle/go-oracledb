...
func newTTIbvc() driverCommon.Message[driverCommon.MessageType] {
	return &tTIbvc{}
}

// GetMsgCode implements Message.GetMsgCode, returning the TTC BVC message code.
func (bvc *tTIbvc) GetMsgCode() driverCommon.MessageType {
	return TTIBVC
}
...
func (bvc *tTIbvc) SetNumberOfColumns(noOfCols driverCommon.UB4) {
	bvc.numberOfColumns = noOfCols
	bvc.bvcColSent = driverCommon.NewBitSet(int(bvc.numberOfColumns))
	bvc.bvcFound = false
}
