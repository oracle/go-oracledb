...
func (m *tTIlob) SetDefinition(lobDef *lobDefinition) error {
	if lobDef == nil {
		...
		return common.NewOracleError(oracleErrors.InvalidLOBBuffer, nil, "marshal", TTCMsgTypeDescription[m.GetMsgCode()], "definition missing")
	}
...
func (m *tTIlob) MarshalTo(ctx context.Context, engine driverCommon.Marshaller) error {
	if !m.definitionConfigured {
		common.Odl.Error("TTILob.MarshalTo: definition not configured")
		return common.NewOracleError(oracleErrors.InvalidLOBBuffer, nil, "marshal", TTCMsgTypeDescription[m.GetMsgCode()], "definition not configured")
	}
	...
	// lobslen is sb4
	if err := engine.MarshalSB4(ctx, driverCommon.SB4(sourceLocLen)); err != nil {
	...
	if destinationLocatorHasBytes {
		m.lobPayloadDefinition.destinationLength = driverCommon.SB4(destinationLocator.length())
	}
	// lobdlen is sb4
	if err := engine.MarshalSB4(ctx, driverCommon.SB4(m.lobPayloadDefinition.destinationLength)); err != nil {
	...
	if err := engine.MarshalUB4(ctx, driverCommon.UB4(m.lobPayloadDefinition.operation)); err != nil {
	...
	sourceOffset := driverCommon.UB8(0)
	...
	destinationOffset := driverCommon.UB8(0)
	...
	if m.lobPayloadDefinition.lobscnl != 0 && len(m.lobPayloadDefinition.lobscn) > 0 {
		if len(m.lobPayloadDefinition.lobscn) < int(m.lobPayloadDefinition.lobscnl) {
			...
			return common.NewOracleError(oracleErrors.FailMarshal, nil, TTCMsgTypeDescription[m.GetMsgCode()])
		}
...
func (m *tTIlob) _marshalPointer(ctx context.Context, engine driverCommon.Marshaller, isNullPointer bool, pointerType string) error {
	var err error
	if isNullPointer {
