...
type ttioallrpa struct {
	...
	cursorId driverCommon.SB4   // from al8o4[2]
	al8o4    []driverCommon.UB4 // raw UB4 vector for reference

	registrationFeedback []byte
	// pidmlRowCounts holds per-iteration DML row counts when present.
	pidmlRowCounts []driverCommon.UB8
}

func (p *ttioallrpa) GetMsgCode() driverCommon.MessageType {
	return TTIRPA
}

func (p *ttioallrpa) getCursorId() driverCommon.SB4 {
	return p.cursorId
}

func newTTIOallRPA() driverCommon.Message[driverCommon.MessageType] {
	return &ttioallrpa{}
}
...
func (p *ttioallrpa) UnmarshalFrom(ctx context.Context, mar driverCommon.Marshaller) error {
	...
	p.al8o4 = make([]driverCommon.UB4, int(al8o4l))
	...
	const KSCNFVB driverCommon.UB4 = 0x8000 // fully valid bit for kscnwrp
	if len(p.al8o4) >= 2 {
		least := p.al8o4[0]
	...
	// Cursor id
	if len(p.al8o4) >= 3 {
		p.cursorId = driverCommon.SB4(p.al8o4[2])
	}
...
func (p *ttioallrpa) UnmarshalDMLRows(ctx context.Context, mar driverCommon.Marshaller) error {
	...
	if count > 0 {
		p.pidmlRowCounts = make([]driverCommon.UB8, int(count))
...
// can be 0
func (p *ttioallrpa) getTotalAffectedRowsCount() driverCommon.UB8 {
	var total driverCommon.UB8 = 0
