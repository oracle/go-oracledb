...
const (
	kpwrnFirst driverCommon.UB2 = 0x01
	kpwrnNext  driverCommon.UB2 = 0x02
	kpwrnLast  driverCommon.UB2 = 0x04
)

// tTIwrn is a non-fatal warning sent by the server. Multiple warning messages
// can be chained using warningFlags.
type tTIwrn struct {
	warningNumber  driverCommon.UB2
	warningMessage string
	warningFlags   driverCommon.UB2
}

func newTTIwrn() driverCommon.Message[driverCommon.MessageType] {
	return &tTIwrn{}
}

func (w *tTIwrn) GetMsgCode() driverCommon.MessageType {
	return TTIWRN
}
