...
func TestDriver_DMLReturning_Merge_InsertGeneratedIdentity(t *testing.T) {
	...
	ctx := context.Background()
	table := createObjectName("t_merge_ret_ident")
	constraint := createObjectName("merge_ret_ident_uk")
	if _, err := db.ExecContext(ctx, `CREATE TABLE `+table+` (
		id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
		external_id NUMBER NOT NULL,
		name VARCHAR2(100),
		CONSTRAINT `+constraint+` UNIQUE (external_id)
	)`); err != nil {
		t.Fatalf("create table failed: %v", err)
	}
	defer func() {
	...
	stmt, err := db.PrepareContext(ctx, `MERGE INTO `+table+` t
USING (SELECT :external_id AS external_id, :name AS name FROM dual) s
ON (t.external_id = s.external_id)
WHEN NOT MATCHED THEN
  INSERT (external_id, name)
  VALUES (s.external_id, s.name)
RETURNING id, external_id, name INTO :out_id, :out_external_id, :out_name`)
	if err != nil {
		t.Fatalf("prepare merge returning failed: %v", err)
	}
	defer stmt.Close()

	var returnedID int64
	var returnedExternalID int64
	var returnedName string
	result, err := stmt.ExecContext(
		ctx,
		sql.Named("external_id", int64(2)),
		sql.Named("name", "merge-insert"),
		sql.Named("out_id", sql.Out{Dest: &returnedID}),
		sql.Named("out_external_id", sql.Out{Dest: &returnedExternalID}),
		sql.Named("out_name", sql.Out{Dest: &returnedName}),
	)
	if err != nil {
		t.Fatalf("exec merge returning failed: %v", err)
	}
	...
	if returnedID <= 0 {
		t.Fatalf("expected generated identity to be returned, got %d", returnedID)
	}
	if returnedExternalID != 2 {
		t.Fatalf("unexpected returned external_id: got %d, want 2", returnedExternalID)
	}
	if returnedName != "merge-insert" {
		t.Fatalf("unexpected returned name: got %q, want %q", returnedName, "merge-insert")
	}

	var persistedID int64
	var persistedExternalID int64
	var persistedName string
	if err := db.QueryRowContext(ctx,
		"SELECT id, external_id, name FROM "+table+" WHERE external_id = :1",
		int64(2),
	).Scan(&persistedID, &persistedExternalID, &persistedName); err != nil {
		t.Fatalf("select persisted merge row failed: %v", err)
	}
	if persistedID != returnedID || persistedExternalID != returnedExternalID || persistedName != returnedName {
		t.Fatalf(
			"persisted merge row mismatch: got (%d, %d, %q), want (%d, %d, %q)",
			persistedID, persistedExternalID, persistedName,
			returnedID, returnedExternalID, returnedName,
		)
	}
}
...
func TestDriver_DMLReturning_PLSQLForallMerge_MixedScalars(t *testing.T) {
	...
	ctx := context.Background()
	table := createObjectName("t_bulk_merge_ret")
	if _, err := db.ExecContext(ctx, `CREATE TABLE `+table+` (
		seq NUMBER GENERATED BY DEFAULT AS IDENTITY,
		folder_id VARCHAR2(64),
		prop_key VARCHAR2(64),
		prop_value VARCHAR2(64),
		CONSTRAINT pk_t_bulk_merge_ret PRIMARY KEY (folder_id, prop_key)
	)`); err != nil {
		t.Fatalf("create table failed: %v", err)
	}
	defer func() {
  ...
  TYPE t_record IS RECORD (
    seq ` + table + `.seq%TYPE,
    folder_id ` + table + `.folder_id%TYPE,
    prop_key ` + table + `.prop_key%TYPE,
    prop_value ` + table + `.prop_value%TYPE
  );
  TYPE t_records IS TABLE OF t_record;
  l_affected_records t_records;
  TYPE t_folder_id_array IS TABLE OF VARCHAR2(64);
  l_folder_id_array t_folder_id_array;
  TYPE t_prop_key_array IS TABLE OF VARCHAR2(64);
  l_prop_key_array t_prop_key_array;
  TYPE t_prop_value_array IS TABLE OF VARCHAR2(64);
  l_prop_value_array t_prop_value_array;
BEGIN
  l_folder_id_array := t_folder_id_array(:folder_id_1, :folder_id_2);
  l_prop_key_array := t_prop_key_array(:prop_key_1, :prop_key_2);
  l_prop_value_array := t_prop_value_array(:prop_value_1, :prop_value_2);

  FORALL i IN 1..2
    MERGE INTO ` + table + ` t
    USING (
      SELECT l_folder_id_array(i) AS folder_id,
             l_prop_key_array(i) AS prop_key,
             l_prop_value_array(i) AS prop_value
      FROM dual
    ) s
    ON (t.folder_id = s.folder_id AND t.prop_key = s.prop_key)
    WHEN MATCHED THEN
      UPDATE SET t.prop_value = s.prop_value
    WHEN NOT MATCHED THEN
      INSERT (folder_id, prop_key, prop_value)
      VALUES (s.folder_id, s.prop_key, s.prop_value)
    RETURNING seq, folder_id, prop_key, prop_value
    BULK COLLECT INTO l_affected_records;

  IF l_affected_records.COUNT > 0 THEN :seq_1 := l_affected_records(1).seq; END IF;
  IF l_affected_records.COUNT > 0 THEN :returned_folder_id_1 := l_affected_records(1).folder_id; END IF;
  IF l_affected_records.COUNT > 0 THEN :returned_prop_key_1 := l_affected_records(1).prop_key; END IF;
  IF l_affected_records.COUNT > 0 THEN :returned_prop_value_1 := l_affected_records(1).prop_value; END IF;
  IF l_affected_records.COUNT > 1 THEN :seq_2 := l_affected_records(2).seq; END IF;
  IF l_affected_records.COUNT > 1 THEN :returned_folder_id_2 := l_affected_records(2).folder_id; END IF;
  IF l_affected_records.COUNT > 1 THEN :returned_prop_key_2 := l_affected_records(2).prop_key; END IF;
  IF l_affected_records.COUNT > 1 THEN :returned_prop_value_2 := l_affected_records(2).prop_value; END IF;
END;`

	var seq1, seq2 int64
	var folderID1, folderID2 string
	var propKey1, propKey2 string
	var propValue1, propValue2 string

	if _, err := db.ExecContext(
		ctx,
		block,
		sql.Named("folder_id_1", "folder-1"),
		sql.Named("folder_id_2", "folder-1"),
		sql.Named("prop_key_1", "foo1"),
		sql.Named("prop_key_2", "foo2"),
		sql.Named("prop_value_1", "bar1"),
		sql.Named("prop_value_2", "bar2"),
		sql.Named("seq_1", sql.Out{Dest: &seq1}),
		sql.Named("returned_folder_id_1", sql.Out{Dest: &folderID1}),
		sql.Named("returned_prop_key_1", sql.Out{Dest: &propKey1}),
		sql.Named("returned_prop_value_1", sql.Out{Dest: &propValue1}),
		sql.Named("seq_2", sql.Out{Dest: &seq2}),
		sql.Named("returned_folder_id_2", sql.Out{Dest: &folderID2}),
		sql.Named("returned_prop_key_2", sql.Out{Dest: &propKey2}),
		sql.Named("returned_prop_value_2", sql.Out{Dest: &propValue2}),
	); err != nil {
		t.Fatalf("exec PL/SQL FORALL MERGE RETURNING failed: %v", err)
	}

	if seq1 <= 0 || seq2 <= 0 {
		t.Fatalf("expected generated identity values for both rows, got seq1=%d seq2=%d", seq1, seq2)
	}
	if folderID1 != "folder-1" || folderID2 != "folder-1" {
		t.Fatalf("unexpected returned folder_ids: got (%q, %q)", folderID1, folderID2)
	}
	if propKey1 != "foo1" || propKey2 != "foo2" {
		t.Fatalf("unexpected returned prop_keys: got (%q, %q)", propKey1, propKey2)
	}
	if propValue1 != "bar1" || propValue2 != "bar2" {
		t.Fatalf("unexpected returned prop_values: got (%q, %q)", propValue1, propValue2)
	}

	rows, err := db.QueryContext(ctx,
		"SELECT seq, folder_id, prop_key, prop_value FROM "+table+" ORDER BY prop_key")
	if err != nil {
		t.Fatalf("select persisted rows failed: %v", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.seq, &row.folderID, &row.propKey, &row.propValue); err != nil {
			t.Fatalf("scan persisted row failed: %v", err)
		}
	...
	if len(persisted) != 2 {
		t.Fatalf("expected 2 persisted rows, got %d", len(persisted))
	}
	if persisted[0].seq != seq1 || persisted[0].folderID != folderID1 || persisted[0].propKey != propKey1 || persisted[0].propValue != propValue1 {
		t.Fatalf("first persisted row mismatch: got %+v, want seq=%d folder_id=%q prop_key=%q prop_value=%q",
			persisted[0], seq1, folderID1, propKey1, propValue1)
	}
	if persisted[1].seq != seq2 || persisted[1].folderID != folderID2 || persisted[1].propKey != propKey2 || persisted[1].propValue != propValue2 {
		t.Fatalf("second persisted row mismatch: got %+v, want seq=%d folder_id=%q prop_key=%q prop_value=%q",
			persisted[1], seq2, folderID2, propKey2, propValue2)
	}
}
