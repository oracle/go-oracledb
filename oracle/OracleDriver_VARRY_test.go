...
func TestDriver_VARRAY_TypeShapeDebug(t *testing.T) {
	...
	var unusedAttributes, unusedSubtypes ttc.RefCursor
...
func TestDriver_VARRAY_GetObjectTypeOtherScalarTypes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Use the public Execer-based API directly; the sql.Conn keeps the
			// descriptor and subsequent bind operations on one physical session.
			var typ *adt.ObjectType
			err = conn.Raw(func(driverConn any) error {
				physical, ok := driverConn.(*ttc.Connection)
				if !ok {
